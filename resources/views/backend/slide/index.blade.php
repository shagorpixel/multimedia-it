...
@section('content')
    <div class=" w-11/12 bg-white mt-6 p-4 rounded-lg mx-auto">
        <div class=" flex items-center justify-between py-3">
            <h2 class=" text-xl font-semibold">Slide List</h2>
            <a class=" inline-block bg-blue-500 text-white font-semibold px-5 py-2 rounded-md"
                href="{{ route('slide.create') }}">Create Slide</a>
        </div>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                                @foreach ($slides as $slide)
                                    <tr>
                                        ...
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200">
                                            <img class="w-20 rounded" src="{{asset('website/image/'.$slide->image)}}" alt="">
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                            ...
                                            <a class=" bg-gray-100 py-1 px-2 rounded inline-block text-blak"
                                                href="{{ route('slide.destroy', $slide->id) }}"><i
                                                    class="fa-regular fa-eye"></i></a>
                                        <form class="inline-block" action="{{route('slide.destroy',$slide->id)}}" method="POST">
                                            @csrf
                                            @method('delete')
